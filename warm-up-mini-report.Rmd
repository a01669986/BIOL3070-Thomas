---
title: "Warm-up mini-Report: COPD"
author: "Kyle Thomas"
date: "`r Sys.Date()`"
output:
  github_document:
...
## Prediction
If low air quality plays an important role is influencing COPD the the states with the low air quality will have higher rates of COPD.
...
This formats our data so R can effectively read and format the states data. First, we define define our pattern and format the COPD data. Second, we create our data frame.
...
```
Creation of the COPD gender maps and colors. We first join the map data and create the color scale. We then map the data.
```{r copd-by-gender-map}
# Get map data
...
```
COPD prevalence is compared between males and females in the continental United States. Females show a slightly higher average of COPD. This is relevant because it shows that gender does not play a large role in COPD rates. This code creates the difference between sexes map. We first differentiate male and female. Second, we plot the difference. Lastly, the color scale is created and the map is printed.

```{r copd-difference-map}
states_map <- map_data("state") 
...
```
COPD percent difference for females compared to males in shown, with most states having a higher percentage of COPD prevalence in females.

## Air Quality Index
This code chunk creates the overall AQI map. We begin by reformatting the raw data. Second, we join the reformatted data to the mapping package. Lastly, we print the map.
```{r air-quality-data}
...
```
Overall AQI is shown for each of the continental United States, averaged over the year 2025. All states land generally within an acceptable range for daily intake, with some on the higher side indicating a lower AQI.

This code normalizes and reformats our data. We begin by loading needed libraries. Then, we load in our data and format to recombine them. Lastly, we normalize our data using the SQRT method.
...
```
This code plots our SQRT and SQRT reflected data. We first create the histogram and colors. Second, we create the axes.
```{r}
...
```
Overall COPD rate percent is shown to be around 6, with some variation among other states.
...
```
Overall AQI is shown to be around 44, with a wide variation among other states.
...
We were able to determine that there is correlation between areas of low air quality having higher rates of lung disease. The correlation isn’t huge, but it is relevant according to our p-value of 0.01793 from our generalized linear model. This shows that poor air quality is associated with development of COPD, but that it may not be the only cause or the biggest cause. Our R² value was 0.1112, which is the proportion of our data that is normalized or combinable. This means that about 11% of variation in COPD rates are associated with poor air quality. We don’t know if there is a genetic tie to COPD in these areas that may be affecting the number of infected individuals. There may also be tainted data due to travel or people who have moved from one state to another. We also learned that there is no difference in COPD levels between genders, with an insignificant p-value of 2.27E-5. While there are still some uncertainties, we can confidently say that areas with lower air quality will have higher rates of Chronic Obstructive Pulmonary Disease.
...
# REFERENCES   
1. American Lung Association. (2023). COPD prevalence rates and counts by state and gender. Retrieved November 18, 2025, from https://www.lung.org/research/trends-in-lung-disease/copd-trends-brief/data-tables/copd-prevalence-rates-by-state-gender

2. Duan, R.R., Hao, K., & Yang, T. (2020). Air pollution and chronic obstructive pulmonary disease. Chronic Diseases and Translational Medicine, 6(4), 260–269. https://doi.org/10.1016/j.cdtm.2020.05.004

3. Google. (2025). Gemini (Version Jan 2025) [Large language model]. Accessed `r Sys.Date()`.

4. OpenAI. (2025). ChatGPT (Jan 2025 version) [Large language model]. Accessed `r Sys.Date()`.

5. World Population Review. (2025). Air Quality by State 2025. https://worldpopulationreview.com/state-rankings/air-quality-by-state
